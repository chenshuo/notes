...
* \(V_\mathrm{R3}\) denotes the voltage across resistor \(R_3\), whish is \(9 \Delta V_\mathrm{BE}\), assuming \(I_\mathrm{C3}\approx I_\mathrm{E3}\).
...
7. Tempco of \(V_\mathrm{R3} + V_\mathrm{R4}\) is \(45.5 \times 86.2\) = 3.922 V/°C, cancelling out negative tempco of \(V_\mathrm{BE1} + V_\mathrm{BE5}\), which is \(\dfrac{V_\mathrm{BE1} - V_\mathrm{g0}}{T_0} +\dfrac{V_\mathrm{BE5} - V_\mathrm{g0}}{T_0}\)
...
1. TODO: analyse the current mirror of gain 0.5, made of \(Q_8\) and \(Q_9\).
